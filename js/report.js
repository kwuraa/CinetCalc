...
export function renderReportProductsList(listContainer, categories, recommendation, selectedCategory = "all") {
  ...
  if (filtered.length > 0) {
    listContainer.innerHTML = filtered.map((prod, i) => {
    ...
    listContainer.querySelectorAll(".report-prod-checkbox").forEach((cb) => {
      cb.addEventListener("change", () => {
        cb.closest(".report-prod-item")?.classList.toggle("is-excluded", !cb.checked);
      });
    });
  } else {
 ...
 */
export function populateReportModal(domElements, reportData, categories, recommendation) {
  const { repVehicle, repMass, repVel, repAngle, repTotalEnergy, repEffectiveEnergy, repReqEnergy, reportProductsList, reportCategoryFilter } = domElements;
  const { vehicleName, mass, velKmh, velMs, angle, totalEnergy, effectiveEnergy, reqEnergy } = reportData;

  if (repVehicle) repVehicle.textContent = vehicleName;
  if (repMass) repMass.textContent = `${mass.toLocaleString("pt-BR")} kg`;
  if (repVel) repVel.textContent = `${velKmh.toFixed(1)} km/h (${velMs.toFixed(2)} m/s)`;
  if (repAngle) repAngle.textContent = `${angle}º ${angle === 90 ? "(Frontal)" : ""}`;
  if (repTotalEnergy) repTotalEnergy.textContent = `${(totalEnergy / 1000).toFixed(2)} kJ`;
  if (repEffectiveEnergy) repEffectiveEnergy.textContent = `${(effectiveEnergy / 1000).toFixed(2)} kJ`;
  if (repReqEnergy) repReqEnergy.textContent = `${(reqEnergy / 1000).toFixed(2)} kJ`;

  const selectedCategory = reportCategoryFilter ? reportCategoryFilter.value : "all";
  renderReportProductsList(reportProductsList, categories, recommendation, selectedCategory);
}
...
export function initReportModal(config) {
  const { modal, btnOpen, btnClose, btnCancel, btnPrint, categoryFilter, btnSelectAll, btnSelectIdeal, productsList, reportDateEl, reportCodeEl, getReportData, getCategories, getRecommendation } = config;
  ...
  const openModal = () => {
    if (!modal) return;
    populateReportModal(config, getReportData(), getCategories(), getRecommendation());
  ...
  if (categoryFilter) {
    categoryFilter.addEventListener("change", () => {
      renderReportProductsList(productsList, getCategories(), getRecommendation(), categoryFilter.value);
    });
  }
  ...
  if (btnSelectIdeal && productsList) {
    btnSelectIdeal.addEventListener("click", () => {
      productsList.querySelectorAll(".report-prod-checkbox").forEach((cb, idx) => {
        cb.checked = idx === 0;
        cb.closest(".report-prod-item")?.classList.toggle("is-excluded", idx !== 0);
      });
    });
  }
}
